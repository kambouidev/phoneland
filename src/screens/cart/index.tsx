...
const CartPage = () => {
    ...
    return (
        <div className="w-full" data-cy="cart-page">
            <div className="flex-1 p-4 md:px-8 lg:px-12 h-[calc(100vh-64px)] overflow-y-auto scrollbar-transparent">
                <div className="text-xl mb-10 uppercase" data-cy="cart-title">cart ({cart.length})</div>

                <div className="grid grid-cols-1 pb-20 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5" data-cy="cart-device-list">
                ...
                <footer className="fixed bottom-0 left-0 right-0 bg-black p-4 md:px-8 lg:px-12">
                    {cart.length > 0 && <div className="flex flex-row gap-4 uppercase text-white justify-between sm:hidden">
                        <span className="text-lg font-semibold" data-cy="cart-total-label">Total</span>
                        <span className="text-lg font-semibold" data-cy="cart-total-amount">{total} EUR</span>
                    </div>}
